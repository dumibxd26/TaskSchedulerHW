...
Lucrul la acest proiect mi-a schimbat intuiția despre ce limitează de fapt performanța scheduler-ului. Înainte de experimente, am imaginat că politicile în sine ar domina: FIFO ar fi simplu dar nedrept, priority ar menține latența scăzută pentru job-urile critice, iar round-robin ar face un compromis între optimalitate și fairness. După instrumentarea serviciilor, a devenit evident că planul de control este primul lucru care se satură. Fiecare apel `next` și `done` este un hop REST prin FastAPI; cu 16 workers × 16 core-uri, scheduler-ul petrece mai mult timp răspunzând la cereri HTTP decât aranjând job-uri. Lecția este că comparațiile teoretice ale politicilor sunt inutile dacă infrastructura nu poate ține pasul.

Schedularea cu prioritate a fost cea mai surprinzătoare. Mă așteptam la starvation clasic pentru job-urile cu prioritate mică, și am văzut într-adevăr latențe de coadă de câteva secunde pentru batch-urile de background. Dar instrumentarea a arătat și că burst-urile cu prioritate mare pot supraîncărca cache-urile L3 și pot crește utilizarea CPU suficient de mult încât să încetinească *toate* job-urile, nu doar pe cele cu prioritate mică. Cu alte cuvinte, favorizarea muncii importante poate avea încă efecte negative dacă implementarea nu limitează sau nu netezește sosirile. Soluția nu a fost algoritmică, ci practică: am adăugat cozi ready bazate pe heap plus thread-uri de sosire pentru a menține admisia sarcinii netedă.

Round-robin a oferit cea mai educativă poveste despre fairness. În dataset-uri cu quantum-uri mici (1–5 ms simulate), numărul de slice-uri per job a explodat, ceea ce înseamnă că numărul de round-trip-uri ale scheduler-ului a crescut liniar cu (job-uri × slice-uri). Acesta este exact versiunea distribuită a overhead-ului de context-switch. Observând `avg_slices_per_job` urcând peste 400, a devenit tangibil că fairness-ul costă lățime de bandă reală. Acum gândesc fairness-ul ca pe o resursă bugetată: fiecare slice suplimentar ar trebui justificat printr-o metrică precum indicele Jain bazat pe slowdown, nu doar prin tradiție.

FIFO, deși aparent naiv, a devenit linia de bază care ne-a ținut cinstiți. Execuția sa într-un singur pas ne-a permis să izolăm cât de mult din inflația latenței în alte politici a venit din coordonare, mai degrabă decât din variabilitatea sarcinii de lucru. În dataset-urile cu burst, a produs acumulare predictibilă de cozi, și pentru că workers rulează job-uri până la finalizare, cantitatea de "chatter" al scheduler-ului a fost minimală. Acest lucru a oferit o condiție de control curată pentru depanarea căilor mai complexe.

Un alt aspect important privește igiena automatizării. `run_matrix.py` a suportat inițial doar un subset mic de configurații, dar extinderea sa la grila carteziană completă a deblocat un flux de lucru mai științific: editează cod, rulează din nou grila, compară snapshot-uri `summary.csv`. Scriptul m-a învățat și să tratez infrastructura (pornirea serverelor uvicorn, așteptarea pentru readiness, copierea CSV-urilor) ca pe cod; copierea și lipirea comenzilor shell era prea fragilă pentru experimentele 16×16. Directoarele numite cu grijă—`results/replicas_X_cores_Y`—acum servesc ca un pachet de date public care satisface cerința cursului pentru partajarea dataset-urilor.

În final, proiectul a clarificat valoarea instrumentării. Endpoint-urile Prometheus și log-urile JSON structurate ne-au permis să corelăm utilizarea CPU a scheduler-ului cu dimensiunile cozilor și ratele de finalizare a job-urilor. Fără aceste semnale, observațiile calitative de mai sus ar fi rămas anecdote. Experiența a consolidat ideea că piesele de "opinie" despre comportamentul sistemului trebuie să fie fundamentate pe dovezi reproducibile, nu doar pe intuiție.

---

### Opinia despre Algoritmul Round-Robin în Sisteme Distribuite

Round-Robin este simplu pe hârtie, dar într-o configurare distribuită implementarea contează aproape la fel de mult ca politica: lock-urile, long-polling-ul, rata de cereri, operațiile pe cozi și logarea rezultatelor adaugă toate overhead real.

Mă așteptam ca timpul de execuție să scadă aproape liniar cu mai multe mașini/core-uri, dar graficele arată randamente descrescătoare, în special cu quantum-uri mici. Scheduler-ul devine un bottleneck pentru că fiecare time slice declanșează coordonare: un worker cere muncă și mai târziu raportează finalizarea. Când quantum-ul este foarte mic, slice-urile per job explodează, ceea ce înseamnă că round-trip-urile HTTP explodează, și sistemul petrece mai mult timp programând/comunicând decât executând. De aceea quantum-urile de 10ms duc la `avg_slices_per_job` foarte mari și la timp de execuție mult mai lung.

Fairness-ul are și el un compromis clar. Quantum-uri mai mici îmbunătățesc de obicei fairness-ul și responsivitatea (job-urile primesc timp CPU mai des), dar cresc preemptările și încărcarea scheduler-ului. Folosirea metricilor precum Gini pentru waiting-time sau indicele Jain bazat pe slowdown a ajutat să arate că "corect" depinde de ce măsurăm: timp de așteptare egal, slowdown egal, sau comportament mai bun la coadă.
